...
from HRIS_App.models import Employee, EmployeeType, Designation, EmployeeGrade, Branch, Region, Cadre
...
def get_filter_counts(request):
    ...
    if not employee_type:  # Only if not already filtered by employee_type
        type_counts = employees.values('employee_type__name').annotate(count=Count('id'))
    ...
    if not employee_grade:  # Only if not already filtered by employee_grade
        grade_counts = employees.values('employee_grade__grade_name').annotate(count=Count('id'))
        for item in grade_counts:
            if item['employee_grade__grade_name']:
                employee_grade_counts[item['employee_grade__grade_name']] = item['count']
    
    # Branch counts
    if not branch:  # Only if not already filtered by branch
        branch_counts_data = employees.values('branch__branch_name').annotate(count=Count('id'))
    ...
    if not region:  # Only if not already filtered by region
        region_counts_data = employees.values('region__name').annotate(count=Count('id'))
...
def export_csv(request):
    ...
    if visible_columns:
        ...
        visible_columns = [
            'SAP_ID', 'name', 'email', 'employee_type', 'designation', 'employee_grade', 'branch',
            'branch_code', 'region', 'pending_inquiry', 'remarks', 'transfer_remarks', 'grade_assignment'
        ]
    ...
    column_mapping = {
        ...
        'employee_grade': {'header': 'Employee Grade', 'field': 'employee_grade__grade_name'},
    ...
    header_row = [column_mapping[col]['header'] for col in visible_columns if col in column_mapping]
    ...
    for employee in employees:
        ...
        for col in visible_columns:
            if col in column_mapping:
                ...
                if field.endswith('__name') or field.endswith('__title') or field.endswith('__grade_name') or field.endswith('__branch_code') or field.endswith('__branch_name'):
